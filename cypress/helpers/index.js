...
export const gotoNthBookInTheList = (nth) => {
  //cy.get('button[data-test="book-item-button"]').contains('View Details').eq(nth).click();
  //cy.get('a').eq(nth).click();
  //cy.get('a').eq(nth).should('be.visible').contains('View Details').eq(nth).click();
  cy.visit('http://localhost:3000/books/1');
}
